...
class ManipulationConfig:
    ...
    grasp_phase_steps: int = 77  # Close Gripper at step
...
class DrawerOpen(IsaacLabViser):
    def __init__(self, *args, **kwargs):
        self.debug_marker_vis = False
        self.dig_config_path = Path(f'{outputs_dir}/drawer/dig/2025-04-02_134836/config.yml')
    ...
    def load_track_data(self):
        ...
        self.part_deltas = torch.flip(self.part_deltas, [0]) # reverse the part deltas
        self.part_deltas = self.part_deltas[10::2]
    ...
    def _reset_object_state(self):
        ...
        random_xyz = torch.randn_like(self.scene.env_origins) * 0.0505
        random_xyz[:, 2] = 0.0
        
        z_rot = torch.rand((self.scene.num_envs,)) * 1.01 + onp.pi/2 - 1  #* 0.8 + onp.pi/2 - 0.4
    ...
    def _handle_manipulation(
        ...
        z_pos_error_mask = (left_z_pos < 0.02) | (right_z_pos < 0.02)
