...
from isaaclab.utils.math import subtract_frame_transforms, euler_xyz_from_quat, apply_delta_pose
...
class PickTiger(IsaacLabViser):
    ...
    def wrist_cam_tf(self):
        ee2wrist_l = vtf.SE3.from_rotation_and_translation(vtf.SO3(onp.array([0, 0, 0, 1])), onp.array([0, 0.05, -0.075]))
        ee2wrist_r = vtf.SE3.from_rotation_and_translation(vtf.SO3(onp.array([1, 0, 0, 0])), onp.array([0, -0.05, -0.075]))
        ee_2wrist_rot = vtf.SE3.from_rotation_and_translation(vtf.SO3(onp.array([0.9848078, -0.1736482, 0, 0])), onp.array([0, 0, 0]))
        c2w_l = vtf.SE3(torch.cat([self.ee_pose_w_left[:, 3:7], self.ee_pose_w_left[:, :3]], dim=-1).cpu().numpy()) @ ee2wrist_l @ ee_2wrist_rot
        c2w_r = vtf.SE3(torch.cat([self.ee_pose_w_right[:, 3:7], self.ee_pose_w_right[:, :3]], dim=-1).cpu().numpy()) @ ee2wrist_r @ ee_2wrist_rot
        c2w_l_tensor = torch.tensor(c2w_l.parameters()).to(self.ee_pose_w_left.device) # wxyz_xyz
        c2w_r_tensor = torch.tensor(c2w_r.parameters()).to(self.ee_pose_w_right.device) # wxyz_xyz
        ...
        if not self.use_viewport and len(self.camera_manager.frustums) > 0: # Batched Rendering for MultiTiledCameraCfg
            if getattr(self.isaac_viewport_camera.cfg, "cams_per_env", None) is not None:
                ...
                if len(self.camera_manager.frustums) > self.isaac_viewport_camera.cfg.cams_per_env:
                    ...
                    if hasattr(self, 'ee_pose_w_left') and hasattr(self, 'ee_pose_w_right'): # Wrist camera pose updating
                        ...
                        # self.camera_manager.frustums[0].position = c2w_l[self.env, 4:].cpu().detach().numpy()
                        # self.camera_manager.frustums[0].wxyz = c2w_l[self.env, :4].cpu().detach().numpy()
                        # self.camera_manager.frustums[1].position = c2w_r[self.env, 4:].cpu().detach().numpy()
                        # self.camera_manager.frustums[1].wxyz = c2w_r[self.env, :4].cpu().detach().numpy()
                    else:
                        xyzs = []
                        wxyzs = []
    ...
    def run_simulator(self):
        ...
        # Create controller
        controller = JaxMPBatchedController(urdf_path=self.urdf_path['robot'], num_envs=self.scene.num_envs)
        ...
        # Simulate physics
        while self.simulation_app.is_running() and self.successful_envs.value < 1500:
            ...
            if hasattr(self, 'robot'):
                self.ee_pose_w_left = self.robot.data.body_state_w[:, robot_entity_cfg.body_ids[0], 0:7] # Per env robot ee_left to world
                self.ee_pose_w_right = self.robot.data.body_state_w[:, robot_entity_cfg.body_ids[1], 0:7] # Per env robot ee_right to world
            ...
            if count > state_machine_keyframe[0]:
                ...
                if count == state_machine_keyframe[2]-6:
                    ...
                    for rigid_object in self.scene.rigid_objects.values():
                        ...
                        ee_pos_w_left = self.robot.data.body_state_w[:, robot_entity_cfg.body_ids[0], 0:7][:,0:3] # Per env robot ee_left to world
                        ee_pos_w_right = self.robot.data.body_state_w[:, robot_entity_cfg.body_ids[1], 0:7][:,0:3] # Per env robot ee_left to world
                ...
                for idx, robot in enumerate(self.scene.articulations.values()):
                    ...
                    ik_commands = torch.zeros(self.scene.num_envs, 7, device=self.robot.device)
                    ...
                    self.ee_pose_w_left = self.robot.data.body_state_w[:, robot_entity_cfg.body_ids[0], 0:7] # Per env robot ee_left to world
                    self.ee_pose_w_right = self.robot.data.body_state_w[:, robot_entity_cfg.body_ids[1], 0:7] # Per env robot ee_right to world
                    root_pose_w = self.robot.data.root_state_w[:, 0:7] # Per env robot base frames to world
                    joint_pos = self.robot.data.joint_pos[:, robot_entity_cfg.joint_ids]
                    
                    # compute frame in root frame
                    ee_pos_b_left, ee_quat_b_left = subtract_frame_transforms(root_pose_w[:, 0:3], root_pose_w[:, 3:7], self.ee_pose_w_left[:, 0:3], self.ee_pose_w_left[:, 3:7])
                    ee_pos_b_right, ee_quat_b_right = subtract_frame_transforms(root_pose_w[:, 0:3], root_pose_w[:, 3:7], self.ee_pose_w_right[:, 0:3], self.ee_pose_w_right[:, 3:7])
                    ...
                    target_poses[:,0,:] = torch.where(self.left_hand_envs.unsqueeze(0).T.repeat(1,7), ik_commands, torch.cat([ee_pos_b_left, ee_quat_b_left], dim=1)).cpu().detach().numpy()
                    target_poses[:,1,:] = torch.where(~self.left_hand_envs.unsqueeze(0).T.repeat(1,7), ik_commands, torch.cat([ee_pos_b_right, ee_quat_b_right], dim=1)).cpu().detach().numpy()
                    ...
                    joints_jmp = controller.compute_ik(target_poses)
                    ...
                    if gripper_close:
                        ...
                        joint_pos_des[self.left_hand_envs,-4:-2] = 0.024
                        joint_pos_des[self.left_hand_envs,-2:] = 0
                        joint_pos_des[~self.left_hand_envs,-2:] = 0.024
                        joint_pos_des[~self.left_hand_envs,-4:-2] = 0
                    ...
                    if getattr(self, 'data_logger', None) is not None:
                        ...
                        robot_data = {
                            ...
                            "joint_angles": torch.cat((self.robot.data.joint_pos[:, :-3], self.robot.data.joint_pos[:, -2].unsqueeze(-1)), dim=1).clone().cpu().detach().numpy(),
            ...
            # Apply randomization to robot starting pose and rigid object pose
                ...
                if count < state_machine_keyframe[0] - 10:
                    for idx, robot in enumerate(self.scene.articulations.values()):
                        ...
                        joint_pos_target = self.robot.data.default_joint_pos + torch.randn_like(self.robot.data.joint_pos) * 0.015
                        joint_pos_target = joint_pos_target.clamp_(
                            self.robot.data.soft_joint_pos_limits[..., 0], self.robot.data.soft_joint_pos_limits[..., 1]
                        )
            ...
            for name in self.urdf_vis.keys():
                ...
                if self.init_viser:
                    joint_dict = {self.robot.data.joint_names[i]: self.robot.data.joint_pos[self.env][i].item() for i in range(len(self.robot.data.joint_pos[0]))} 
                    self.urdf_vis[name].update_cfg(joint_dict)
                    if self.debug_marker_vis:
                        # obtain quantities from simulation
                        # self.ee_pose_w_left = self.robot.data.body_state_w[:, robot_entity_cfg.body_ids[0], 0:7]
                        # self.ee_pose_w_right = self.robot.data.body_state_w[:, robot_entity_cfg.body_ids[1], 0:7]
                        if hasattr(self, 'left_hand_envs'):
                            ee_pose_w_agg = torch.where(self.left_hand_envs.unsqueeze(0).T.repeat(1,7), self.ee_pose_w_left, self.ee_pose_w_right)
                        else:
