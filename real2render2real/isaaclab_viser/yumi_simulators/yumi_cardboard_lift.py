...
class ManipulationConfig:
    ...
    setup_phase_steps: int = 13  # End of initial setup, begin trajectory
    grasp_phase_steps: int = 80  # Close Gripper at step
    release_offset_steps: int = 0  # Steps in trajectory after release
    ...
    def update(self, count: int):
        ...
        if count > self.config.setup_phase_steps:
            self.ee_goal_offset[2] = self._interpolate_height(
                count,
                self.config.setup_phase_steps + 37,
                self.config.grasp_phase_steps - 3,
                self.config.ee_retracts['start'],
                self.config.ee_retracts['grasp']
            )
            
        if count > self.config.grasp_phase_steps - torch.randint(4, 7, (1,)):
            self.l_gripper_closed = True
            
        if count > self.config.grasp_phase_steps - torch.randint(4, 7, (1,)):
            self.r_gripper_closed = True
...
class CardboardLift(IsaacLabViser):
    ...
    def run_simulator(self):
        ...
        isaaclab2jaxmp_joint_mapping = [1, 3, 5, 7, 9, 11, 13, 0, 2, 4, 6, 8, 10, 12, -2, -4]

        home_pose = self.scene.articulations['robot'].data.default_joint_pos[0].cpu().detach().numpy()
        home_pose = home_pose[isaaclab2jaxmp_joint_mapping]
        self.controller = JaxMPBatchedController(
            urdf_path=self.urdf_path['robot'], 
            num_envs=self.scene.num_envs,
            num_ees=2,
            target_names=["left_dummy_joint", "right_dummy_joint"],
            home_pose = home_pose
        )
    ...
    def load_track_data(self):
        ...
        self.part_deltas = torch.tensor(data["part_deltas"]).cuda()[13:32]
    ...
    def _reset_object_state(self):
        ...
        random_xyz = (torch.rand_like(self.scene.env_origins) * 2 -1) * 0.09
        random_xyz[:, 2] = 0.02
        
        z_rot = (torch.rand((self.scene.num_envs,))*2-1) * 0.8 #+ onp.pi/2 #* 0.8 + onp.pi/2 - 0.4
        ...
        for group_idx, rigid_object in enumerate(self.scene.rigid_objects.values()):
            ...
            if self.grasped_obj_loc_augment and group_idx in self.grasped_idxs: # Object init randomization + Object trajectory interpolation
                new_starts = generate_directional_starts(self.part_deltas[:, group_idx], self.scene.num_envs, magnitude=0.09, direction_weight=0.7, perp_variation=0.15)
    ...
    def _handle_manipulation(
        ...
        z_pos_error_mask = (left_z_pos < 0.045) | (right_z_pos < 0.045)
    ...
    def _target_state_error(
        ...
        pos_error_threshold = 0.03  # Position error threshold in meters
        quat_error_threshold = 0.45  # Quaternion error threshold in radians

        # Position errors (Euclidean distance)
        left_pos_error = (left_target[:,:3] - left_current[:,:3]).norm(dim=-1)

        right_pos_error = (right_target[:,:3] - right_current[:,:3]).norm(dim=-1)
        
        left_quat_error = (2 * torch.arccos(torch.abs(torch.sum(left_target[:,3:7] * left_current[:,3:7], dim=-1)).clamp(-1, 1))
                           )
        right_quat_error = (2 * torch.arccos(torch.abs(torch.sum(right_target[:,3:7] * right_current[:,3:7], dim=-1)).clamp(-1, 1)))
    ...
    def _calculate_target_poses(
        ...
        grasp_tf_0 = tf.SE3(torch.cat([torch.tensor(self.grasp_data[0][0]['orientation']), torch.tensor(self.grasp_data[0][0]['position'])], dim=-1).to(self.scene.env_origins.device).unsqueeze(0))
        grasp_tf_1 = tf.SE3(torch.cat([torch.tensor(self.grasp_data[0][1]['orientation']), torch.tensor(self.grasp_data[0][1]['position'])], dim=-1).to(self.scene.env_origins.device).unsqueeze(0))
        ...
        if self.grasp_perturb is not None:
            if (count >= self.state_machine.config.grasp_phase_steps and 
                ...
                perturbed_grasp_tf_0 = grasp_tf_0 @ self.grasp_perturb @ perturb_tf_x @ perturb_tf_y # Per timestep grasp perturbation
                perturbed_grasp_tf_1 = grasp_tf_1 @ self.grasp_perturb @ perturb_tf_x @ perturb_tf_y # Per timestep grasp perturbation
            else:
                perturbed_grasp_tf_0 = grasp_tf_0 @ self.grasp_perturb
                perturbed_grasp_tf_1 = grasp_tf_1 @ self.grasp_perturb
            ee_target_0 = obj_tf @ perturbed_grasp_tf_0 @ goal_offset_tf
            ee_target_1 = obj_tf @ perturbed_grasp_tf_1 @ goal_offset_tf
        else:
            ee_target_0 = obj_tf @ grasp_tf_0 @ goal_offset_tf
            ee_target_1 = obj_tf @ grasp_tf_1 @ goal_offset_tf
        ...
        command_xyzs_0 = ee_target_0.wxyz_xyz[:, 4:]
        command_wxyzs_0 = ee_target_0.wxyz_xyz[:, :4]
        command_xyzs_1 = ee_target_1.wxyz_xyz[:, 4:]
        command_wxyzs_1 = ee_target_1.wxyz_xyz[:, :4]
        
        # Create target poses array
        target_poses = onp.zeros((self.scene.num_envs, 2, 7))
        target_poses[:,0,:] = torch.where(
            torch.ones_like(self.left_hand_envs.unsqueeze(0).T.repeat(1,7)),
            torch.cat([command_xyzs_1, command_wxyzs_1], dim=1),
            torch.cat([ee_pos_b_left, ee_quat_b_left], dim=1)
        ).cpu().detach().numpy()
        target_poses[:,1,:] = torch.where(
            torch.ones_like(~self.left_hand_envs.unsqueeze(0).T.repeat(1,7)),
            torch.cat([command_xyzs_0, command_wxyzs_0], dim=1),
    ...
    def _set_gripper_positions(self, joint_pos_des: torch.Tensor):
        ...
        if self.state_machine.l_gripper_closed:
            # Left hand
        ...
        if self.state_machine.r_gripper_closed:
            # Right hand
            joint_pos_des[~self.left_hand_envs,-2:] = self.grasp_data[0][self.grasp_idx]['width']
            joint_pos_des[~self.left_hand_envs,-4:-2] = self.grasp_data[0][self.grasp_idx]['width']
        else:
    ...
    def _log_data(self, count: int):
        ...
        robot_data = {
            ...
            "gripper_binary_cmd": torch.cat([
                torch.bitwise_and(
                    torch.tensor(self.state_machine.l_gripper_closed).repeat(
                        self.scene.num_envs
                    ).to(self.left_hand_envs.device), 
                    torch.ones_like(self.left_hand_envs)    
                ).unsqueeze(0).T,
                torch.bitwise_and(
                    torch.tensor(self.state_machine.r_gripper_closed).repeat(
                        self.scene.num_envs
                    ).to(self.left_hand_envs.device),
                    torch.ones_like(~self.left_hand_envs)
                ).unsqueeze(0).T
