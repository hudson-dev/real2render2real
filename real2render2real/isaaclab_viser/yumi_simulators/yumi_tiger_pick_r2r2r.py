...
class ManipulationConfig:
    ...
    setup_phase_steps: int = 15  # End of initial setup, begin trajectory
    grasp_phase_steps: int = 96  # Close Gripper at step
    release_offset_steps: int = 1  # Steps in trajectory after release
...
class ManipulationStateMachine:
    ...
    def update(self, count: int):
        ...
        if count > self.config.setup_phase_steps:
            self.ee_goal_offset[2] = self._interpolate_height(
                count,
                self.config.setup_phase_steps + 8,
                self.config.grasp_phase_steps - 2,
        ...
        if count > self.config.grasp_phase_steps - 9:
    ...
    def _randomize_ee_pose_offset(self):
        ...
        self.ee_rand_goal_offset = ((torch.rand((2,))*2-1)*0.19).tolist()

class TigerPickR2R2R(IsaacLabViser):
    def __init__(self, *args, **kwargs):
        self.debug_marker_vis = False
        # TODO: update dig_config_path
        self.dig_config_path = Path(f'{outputs_dir}/tiger/dig/2025-04-18_001541/config.yml')
    ...
    def run_simulator(self):
        ...
        while self.simulation_app.is_running() and self.successful_envs.value < 1000:
            ...
            if count > self.state_machine.config.setup_phase_steps:
                ...
                if count > self.state_machine.config.setup_phase_steps + 9:
                    self._log_data(count-9)
            else:
    ...
    def load_track_data(self):
        ...
        self.part_deltas = self.part_deltas[4:]
    ...
    def _reset_object_state(self):
        ...
        random_xyz = (torch.rand_like(self.scene.env_origins) * 2 - 1) * 0.23 
        random_xyz[:, 2] = 0.005 + (torch.rand(1).item() * 2 - 1) * 0.0005
        
        z_rot = torch.rand((self.scene.num_envs,)) * 2 * onp.pi 
    ...
    def _handle_manipulation(
        ...
        z_pos_error_mask = (left_z_pos < 0.015) | (right_z_pos < 0.015)
    ...
    def _update_robot_manipulation(
        ...
        if count == self.state_machine.config.setup_phase_steps + 1:
            ...
            # self.left_hand_envs = root_state_b[:, 1] > 0.0
    ...
    def _calculate_target_poses(
        ...
        if count == self.state_machine.config.setup_phase_steps + 1:
            ...
            self.first_ee_quat_b_left = ee_quat_b_left.clone()
            self.first_ee_pos_b_right = ee_pos_b_right.clone()
            self.first_ee_quat_b_right = ee_quat_b_right.clone()
        
        obj_tf = tf.SE3(torch.cat([obj_wxyzs, obj_xyzs], dim=-1))

        grasp_tf = tf.SE3(torch.cat([torch.tensor([0, -0.7316889, 0.6816388, 0]), torch.tensor(self.grasp_data[0][self.grasp_idx]['position'])], dim=-1).to(self.scene.env_origins.device).unsqueeze(0))
        ...
        target_poses[:,0,:] = torch.where(
            ...
            torch.cat([self.first_ee_pos_b_left, self.first_ee_quat_b_left], dim=1)
        ).cpu().detach().numpy()
        target_poses[:,1,:] = torch.where(
            ~self.left_hand_envs.unsqueeze(0).T.repeat(1,7),
            torch.cat([command_xyzs, command_wxyzs], dim=1),
            torch.cat([self.first_ee_pos_b_right, self.first_ee_quat_b_right], dim=1)
        ).cpu().detach().numpy()
    ...
    def _ik_wrapped(
        ...
        joint_pos_des[self.left_hand_envs, right_arm_indices] = current_joint_pos[self.left_hand_envs, right_arm_indices]
        
        # For right-handed environments, replace left arm joint positions with current positions
        joint_pos_des[~self.left_hand_envs, left_arm_indices] = current_joint_pos[~self.left_hand_envs, left_arm_indices]
