...
@dataclass
class Config:
    raw_dataset_folders: List[str] = field(default_factory=lambda: ["/home/yujustin/xi/output_data/yumi_coffee_maker/ablation/background/successes"])
    language_instructions: List[str] = field(default_factory=lambda: ["put the white cup on the coffee machine"])
    output_dir: Path = field(default_factory=lambda: Path("/home/yujustin/xi/output_data/yumi_coffee_maker/ablation/background/dpgs_sim_coffee_maker_1k_background_rand_fast_new"))
...
def process_episode(
    ...
    for step in range(seq_length):
        ...
        action_t[-2:] = proprio_data[step + 1][-2:]  # absolute gripper
